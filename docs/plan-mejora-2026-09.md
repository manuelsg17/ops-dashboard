...
- **Integrada en la rama local `prototipo` (24-sep)**: capa suave global + Metas (Elegida), Rendimiento (B · Suave, anillos `progressRing`), Calculadora (planilla viva + atajos, `domain/calcPlanilla.ts`), Configuración (B · Suave), botón sol/luna, y paginación con orden total (`fecha,id`). Huella vs Ola 7: 0 cifras cambiadas ni quitadas; 66 nuevas (Real AD de la Calculadora); solo difieren 4 deltas de Metas diario "vs mismo punto", que dependen de la fecha de hoy (el baseline es del día anterior). 537 tests.
- Decisión de Manuel (24-sep): en la Calculadora, una celda fijada a mano NO redistribuye el resto (opción 1, "dejarlo como está"); el cuadre avisa "No cuadra" y el KAM ajusta. No implementar el reajuste automático del prototipo.

## 10. Fase 9 (24-sep-2026, rama local `fase9`, sin publicar)
- Calculadora: sección "TukTuk (% de PnL)" visible con estado "Sin declarar"/"Declarado", totales TukTuk en el cuadre, sub-líneas rotuladas (`domain/tkResumen.ts`).
- Metas: aviso de cobertura de cuota TukTuk por KAM + "de eso TukTuk X" con los valores guardados (`domain/cuotaTk.ts`); nunca se estima (regla de Manuel).
- Escala mensual con el mes en curso: número grande = valor del mes de la meta, "Mes en curso" en vez de variación contra el mes completo, proyección de N+R/Horas extrapolada con corte = fin de la última semana cargada (`domain/diasMesEnCurso.ts`); una sola regla (`diasMesReporteDe`) para Metas, Rendimiento, deck y portal. Proyecciones redondeadas.
- Pendientes para Manuel: % de la pestaña TukTuk (hoy 275%: actual de todas las cuentas vs cuota de un KAM); variaciones de ciudad/KAM/productividad en mensual con el mes en curso (comparan contra el mes completo).
